...
namespace WpfApp1.Command.Command_PDF3024
{
    public class HOP_PDF_ViewModel:BaseViewModel
    {
        ...
        public HOP_PDF_ViewModel( ManualResetEventSlim pauseEvent, SemaphoreSlim semaphore, Action<string> addLog, Action<string> updateState, Action<string> updateBLState)
        {
        ...
        private async void OutVoltOperation()
        {
            try
            {
                ...
                await Task.Run(new Action(async () =>
                {
                    ...
                    {
                        receive = await AppServices.CurrentBlueTooth.SendBLSettingCommand("V", OutVolt_Inputs,7);
                        UpdateBLState("设置指令已经执行完");
                    }
                    else if(SerialCommunicationService.IsOpen())
                    {
                        receive = SerialCommunicationService.SendSettingCommand("V", OutVolt_Inputs);
        ...
        //满载有功功率
        private string _FullloadPwr;

        public string FullloadPwr
        {
            get { return _FullloadPwr; }
            set
            {
                _FullloadPwr = Tools.RemoveLeadingZeros(value);
                this.RaiseProperChanged(nameof(FullloadPwr));
            }
        }


        private bool FullloadPwr_IsWorking;


        //设置值
        private string _FullloadPwr_Inputs;

        public string FullloadPwr_Inputs
        {
            get { return _FullloadPwr_Inputs; }
            set
            {
                _FullloadPwr_Inputs = value;
                this.RaiseProperChanged(nameof(FullloadPwr_Inputs));
                Command_SetFullloadPwr.RaiseCanExecuteChanged();
            }
        }


        public RelayCommand Command_SetFullloadPwr { get; }

        /// <summary>
        /// 点击设置
        /// </summary>
        private async void FullloadPwrOperation()
        {
            try
            {
                FullloadPwr_IsWorking = true;
                // 禁用按钮
                Command_SetFullloadPwr.RaiseCanExecuteChanged();
                ...
                await Task.Run(new Action(() =>
                {
                    //执行设置指令
                    Thread.Sleep(2000);//没有这个延时会报错
                    string receive = SerialCommunicationService.SendSettingCommand("设置指令", FullloadPwr_Inputs);
