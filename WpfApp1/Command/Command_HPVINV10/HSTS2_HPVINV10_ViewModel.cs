...
{
    public class HSTS2_HPVINV10_ViewModel: BaseViewModel
    {
        //指令
        ...
        public HSTS2_HPVINV10_ViewModel(ManualResetEventSlim pauseEvent, SemaphoreSlim semaphore, Action<string> addLog, Action<string> _updateState)
        {
        ...
        #region 充电限流标志
        private string _ChargeCurrentLimitFlag;

        public string ChargeCurrentLimitFlag
        {
            get { return _ChargeCurrentLimitFlag; }
            set
            {
                _ChargeCurrentLimitFlag = value;
                this.RaiseProperChanged(nameof(ChargeCurrentLimitFlag));
            }
        }
        #endregion

        #region 市电耦合限流标志
        private string _ACCouplingCurrentLimitFlag;

        public string ACCouplingCurrentLimitFlag
        {
            get { return _ACCouplingCurrentLimitFlag; }
            set
            {
                _ACCouplingCurrentLimitFlag = value;
                this.RaiseProperChanged(nameof(ACCouplingCurrentLimitFlag));
            }
        }
        #endregion

        #region Mppt限流标志
        private string _MPPTCurrentLimitFlag;

        public string MPPTCurrentLimitFlag
        {
            get { return _MPPTCurrentLimitFlag; }
            set
            {
                _MPPTCurrentLimitFlag = value;
                this.RaiseProperChanged(nameof(MPPTCurrentLimitFlag));
            }
        }
        #endregion

        #region 电池放电限流标志
        private string _BatteryDischargeCurrentLimitFlag;

        public string BatteryDischargeCurrentLimitFlag
        {
            get { return _BatteryDischargeCurrentLimitFlag; }
            set
            {
                _BatteryDischargeCurrentLimitFlag = value;
                this.RaiseProperChanged(nameof(BatteryDischargeCurrentLimitFlag));
            }
        }
        #endregion

        #region B路光伏电压状态
        private string _BPhotovoltaicVoltageStatus;

        public string BPhotovoltaicVoltageStatus
        {
            get { return _BPhotovoltaicVoltageStatus; }
            set
            {
                _BPhotovoltaicVoltageStatus = value;
                this.RaiseProperChanged(nameof(BPhotovoltaicVoltageStatus));
            }
        }
        #endregion

        #region B路MPPT状态
        private string _BMPPTStatus;

        public string BMPPTStatus
        {
            get { return _BMPPTStatus; }
            set
            {
                _BMPPTStatus = value;
                this.RaiseProperChanged(nameof(BMPPTStatus));
            }
        }
        #endregion

        #region 当前第二输出远程控制状态
        private string _CurrentSecondOutputRemoteControlStatus;

        public string CurrentSecondOutputRemoteControlStatus
        {
            get { return _CurrentSecondOutputRemoteControlStatus; }
            set
            {
                _CurrentSecondOutputRemoteControlStatus = value;
                this.RaiseProperChanged(nameof(CurrentSecondOutputRemoteControlStatus));
            }
        }
        #endregion

        #region 当前所有输出远程控制状态
        private string _CurrentALLOutputRemoteControlStatus;

        public string CurrentALLOutputRemoteControlStatus
        {
            get { return _CurrentALLOutputRemoteControlStatus; }
            set
            {
                _CurrentALLOutputRemoteControlStatus = value;
                this.RaiseProperChanged(nameof(CurrentALLOutputRemoteControlStatus));
            }
        }
        #endregion
        ...
        public void AnalyseStringToElement(string value)
        {
            ...
            try
            {
                ...
                //充电限流标志
                ChargeCurrentLimitFlag = Values[2].Substring(5, 1);
                //市电耦合限流标志
                ACCouplingCurrentLimitFlag = Values[2].Substring(6, 1);
                //Mppt限流标志
                MPPTCurrentLimitFlag = Values[2].Substring(7, 1);
                //电池放电限流标志
                BatteryDischargeCurrentLimitFlag = Values[2].Substring(8, 1);
                //B路光伏电压状态
                BPhotovoltaicVoltageStatus = Values[3].Substring(0, 1);
                //B路  MPPT状态
                BMPPTStatus = Values[3].Substring(1, 1);
                //当前第二输出远程控制状态
                CurrentSecondOutputRemoteControlStatus = Values[3].Substring(2, 1);
                //当前所有输出远程控制状态
                CurrentALLOutputRemoteControlStatus = Values[3].Substring(3, 1);
            }
            catch (Exception)
            {
                //异常
        ...
        private void ReceiveException(string exceptionDescription)
        {
            ...
            //充电限流标志
            ChargeCurrentLimitFlag = exceptionDescription;
            //市电耦合限流标志
            ACCouplingCurrentLimitFlag = exceptionDescription;
            //Mppt限流标志
            MPPTCurrentLimitFlag = exceptionDescription;
            //电池放电限流标志
            BatteryDischargeCurrentLimitFlag = exceptionDescription;
            //B路光伏电压状态
            BPhotovoltaicVoltageStatus = exceptionDescription;
            //B路  MPPT状态
            BMPPTStatus = exceptionDescription;
            //当前第二输出远程控制状态
            CurrentSecondOutputRemoteControlStatus = exceptionDescription;
            //当前所有输出远程控制状态
            CurrentALLOutputRemoteControlStatus = exceptionDescription;
        }
    }
}
